...
const loadVectorsToPinecone = async (embeddings: Embeddings, docs: Document[]) => {
  while (true) {
    ...
    // console.log(vectorStore.docstore._docs.get('100'));
    // console.log(docs[100]);
    if (JSON.stringify(vectorStore.docstore._docs.get('100')) !==
      JSON.stringify(docs[100])) {
      throw new Error(`HNSWLib vector store doc \n${vectorStore.docstore._docs.get('100')} \n doesn't match \n${docs[100]}`)
    }
